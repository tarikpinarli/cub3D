...
static uint32_t get_pixel_color(mlx_texture_t *tex, int tex_x, int tex_y, double distance)
{
    t_pixel_color    color;
    int             index;
    uint8_t         *pixels;
    ...
    // Increased multiplier from 0.1 to 0.2 for stronger darkness effect
    shade = 1.0 / (1.0 + distance * 0.2);  
    if (shade > 1.0)
        shade = 1.0;
    // Lowered minimum shade from 0.2 to 0.1 for darker distant walls
    if (shade < 0.1)
        shade = 0.1;  
    ...
    // Apply shading to each color component
